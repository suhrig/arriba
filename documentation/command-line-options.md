...
`-r FILE`
: BAM file with read-through alignments as generated by `extract_read-through_fusions`.
...
Run during alignment (see section [Execution](execution.md#extract_read-through_fusions) for a detailed explanation of this use case):
...
`-i FILE`
: Input BAM file containing the normal alignments produced by STAR (`Aligned.sortedByCoord.out.bam`). The file does not need to be sorted. `extract_read-through_fusions` can process unsorted BAM files. Default: `STDIN`

`-o FILE`
: Output BAM file containing all the reads (fragments), which cross the boundaries of a gene. Default: `STDOUT`
