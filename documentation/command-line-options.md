...
`-c FILE`
: SAM/BAM/CRAM file with chimeric alignments as generated by STAR (`Chimeric.out.sam`). The file does not need to be sorted. 

`-r FILE`
: SAM/BAM/CRAM file with read-through alignments as generated by `extract_reads`. The file does not need to be sorted.

`-x FILE`
: BAM/CRAM file with normal alignments as generated by STAR (`Aligned.sortedByCoord.out.bam`). The file must be sorted by coordinate and an index with the file extension `.bai`/`.crai` must be present. The file is used to estimate the mate gap distribution and to filter fusions with no expression around the breakpoints, which are likely false positives.
...
`-x FILE`
: Input file containing the normal alignments from STAR (`Aligned.out.bam`). SAM/BAM/CRAM format are accepted. The file does not need to be sorted. `extract_reads` can process unsorted BAM files. The parameter `-a` must be specified, if CRAM data is processed. Default: `STDIN`

`-r FILE`
: Output file containing reads (fragments) which support read-through fusions, i.e., which cross the boundaries of a gene. The file has the same format (SAM/BAM/CRAM) as the input file given as an argument to the parameter `-x`.
...
`-c FILE`
: Output file containing chimeric reads (fragments), i.e., discordant mates and split reads with supplementary alignments. The file has the same format (SAM/BAM/CRAM) as the input file given as an argument to the parameter `-x`.

`-a FILE`
: Assembly in FastA format. This option is only required when CRAM files are processed. An index with the file extension `.fai` must exist.
