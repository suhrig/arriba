...
unsigned int filter_pcr_fusions(fusions_t& fusions, const chimeric_alignments_t& chimeric_alignments, const float high_expression_quantile, const gene_annotation_index_t& gene_annotation_index) {
	...
	// genes fused during polymerase chain reaction (PCR)/reverse transcription (RT)
	...
	// PCR/RT fusions are mostly observed between highly expressed genes
	...
	for (fusions_t::iterator fusion = fusions.begin(); fusion != fusions.end(); ++fusion) {
		...
		// => count the number of potentially PCR/RT-mediated breakpoints and
		//    penalize events with one/two PCR/RT-mediated breakpoints more
		float potential_pcr_breakpoints = 0;
		if (!fusion->second.exonic1) {
			potential_pcr_breakpoints += 0.5; // intron => unclear if it is a PCR/RT artifact (not likely, but possible)
		} else if (!fusion->second.spliced1) {
			potential_pcr_breakpoints += 1; // breakpoint inside exon => very likely PCR/RT artifact
		} // else: splice-site => probably from splicing and not PCR/RT-mediated
		...
		// PCR/RT artifacts usually have no/few split reads
		...
		// we take the min of both counts, because PCR/RT artifacts often have clipped reads on only one side
		...
		// check if the event exhibits the characteristics of a PCR/RT-mediated fusion
		if (
			// PCR/RT artifacts often have very few split reads
			...
			// PCR/RT artifacts often have many discordant mates, but only few split reads
			...
			// PCR/RT artifacts occur between highly expressed genes
			...
			// PCR/RT artifacts typically have breakpoints inside exons, otherwise they might be true events
