...
unsigned int filter_pcr_fusions(fusions_t& fusions, const annotation_t& gene_annotation, const float max_pcr_fusion_score, const unsigned int max_exonic_breakpoints, const unsigned int max_partners_with_many_exonic_breakpoints, const unsigned int min_split_reads) {
	...
	for (fusions_t::iterator i = fusions.begin(); i != fusions.end(); ++i) {
		if (i->second.gene1 != i->second.gene2 && // there are often many breakpoints within the same gene (probably hairpin fusions)
		    !i->second.spliced1 && !i->second.spliced2 && // breakpoints at splice sites are almost exclusively a result of splicing and thus, no PCR-mediated fusions
		    (!i->second.overlap_duplicate1 || !i->second.overlap_duplicate2) && // don't count breakpoints twice, if they are in overlapping genes
		    i->second.exonic1 && i->second.exonic2 && // PCR fusions only contain spliced transcripts, so we ignore intronic/intergenic breakpoints
...
//TODO how about we don't care if it is spliced in the PCR amplified gene and only care about if it is spliced in the other gene?
		if ((i->second.split_reads1 + i->second.split_reads2 < i->second.discordant_mates || i->second.split_reads1 + i->second.split_reads2 <= min_split_reads) &&
		    (!i->second.spliced1 && !i->second.spliced2 || i->second.split_reads1 + i->second.split_reads2 <= min_split_reads || pcr_fusion_scores[i->second.gene1] >= max_pcr_fusion_score && pcr_fusion_scores[i->second.gene2] >= max_pcr_fusion_score) && // discard fusions, which are not spliced / have few reads / are between genes with high PCR fusion scores
