...
class cigar_t: public vector<uint32_t> {
	public:
		...
		cigar_t operator=(const bam1_t* bam_record) { this->resize(bam_record->core.n_cigar); for (int i = 0; i < bam_record->core.n_cigar; ++i) { (*this)[i] = bam_get_cigar(bam_record)[i]; }; return *this; };
};
